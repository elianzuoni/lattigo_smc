...
func NewRelinearizationKey(n *onet.TreeNodeInstance) (instance onet.ProtocolInstance, e error) {
	...
	if e := p.RegisterChannels(&p.ChannelParams, &p.ChannelSk, &p.ChannelW,&p.ChannelRoundOne,&p.ChannelRoundTwo,&p.ChannelRoundThree); e != nil {
...
func (rlp *RelinearizationKeyProtocol) RelinearizationKey() (bfv.EvaluationKey, error){
	...
	rkg.GenShareRoundOne(u,sk.Get(),rlp.a,r1)
	...
	//now we do r2
	rkg.GenShareRoundTwo(r1,sk.Get(),rlp.a,r2)
	if !rlp.IsLeaf(){
		for _ = range rlp.Children(){
